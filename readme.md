...
Nara — the onboard AI agent for Liquid Galaxy. Built on Hermes Agent.

![LG Logo](./assets/img2.png)
...
Automated Storytelling & Guided Tours: Generate KML narratives and display on the rig
Weather Monitoring: Fetch live weather data and visualize via KML
Efficient Knowledge: Scraped LG Wiki for agent RAG on LG questions
Interactive LG Troubleshooting: Automated debugging of LG issues (builds on stage 4)
Contributor Automator: Generate newcomer tasks, auto-check submissions, give feedback
News & Geopolitical Visualization: Live event mapping on LG
Open Data Visualization: Air, sea, and space data sources
...
A profile is a self-contained Hermes home directory. Starting with one profile (liquid-galaxy-agent) — multiple profiles can be added later.
...
References: Adding Tools | Creating Skills
...
With Hermes Desktop (recommended for macOS / Windows): Download the Hermes Desktop installer from the website and run it.
...
pip install: Code lives at: Python site-packages; hermes binary: ~/.local/bin/hermes; Data directory: ~/.hermes/
Per-user (git installer): Code lives at: ~/.hermes/hermes-agent/; hermes binary: ~/.local/bin/hermes (symlink); Data directory: ~/.hermes/
Root-mode (sudo curl …): Code lives at: /usr/local/lib/hermes-agent/; hermes binary: /usr/local/bin/hermes; Data directory: /root/.hermes/
...
Documentation For detailed installation instructions, usage guides, and developer documentation, visit: https://hermes-agent.nousresearch.com/docs/
...
KML Generation: Description: Geospatial visualizations from natural language
Knowledge Assistant: Description: LG Wiki RAG for setup, troubleshooting, LG ecosystem
Real-Time Visualization: Description: OpenSky, Celestrak, weather/disaster feeds → live KML
Storytelling & Tours: Description: Narrative KML sequences for educational topics
Diagnostics: Description: Health checks on LAN, CPU, Earth, NetworkLinks
Contributor Support: Description: Onboarding, task guidance, submission prep
...
Act first, explain briefly. Fail loud, not silently.
A failure in one skill doesn't crash the session.
Prefer local inference; use remote APIs only when needed.
Log all actions, errors, and data sources used.
...
![hardware img](/assets/img1.png)

Hardware Setup
Step 1 — Attach NVMe SSD via M.2 HAT

Power off and unplug the Raspberry Pi 5.
Attach the Raspberry Pi M.2 HAT+ to the RPi 5 using the PCIe FPC ribbon cable — connect it to the PCIe FPC connector on the bottom edge of the RPi 5 board. Secure the HAT with the provided standoffs.
Insert the 512GB NVMe SSD (M.2 2280 or 2242, M-key) into the M.2 slot on the HAT. Secure it with the retention screw.
Confirm the HAT sits flat and all connectors are fully seated before proceeding.
...
Connect a monitor, keyboard, and mouse (or use SSH after boot).
Connect power — the RPi 5 will boot from the SD card.
Complete the initial OS setup if the desktop wizard appears.
Open a terminal and run a system update:

sudo apt update && sudo apt full-upgrade -y

sudo reboot

After reboot, verify the NVMe is detected:
...
Open Raspberry Pi Imager from the desktop (or run rpi-imager).
Configure the same settings as Step 2 (or reuse your saved customizations).
Storage: Select the NVMe drive (/dev/nvme0n1 — typically listed as the 512GB drive).
...
After the reboot, power off the Raspberry Pi 5:

sudo poweroff

Remove the microSD card.
Power the RPi 5 back on.
The system should now boot directly from the NVMe SSD.
Verify with:
...
Network Info: Helper Script: hostname -I (inline); Scope: lg1 only; Confirm?: No
...
VM / Reverse Tunnel — LG runs on a VM behind a laptop. Tunnel via ssh -N -R 2222:192.168.53.3:22 nara@<pi-ip>.
→ SSH_DEST="lg@localhost -p 2222"

Direct LAN — Real LG hardware on the same network.
...
VM / Reverse Tunnel: SSH Target: lg@localhost -p 2222; Verification: `ss -tlnp \
Direct LAN: SSH Target: lg@; Verification: Ping + SSH to master IP
...
lg-reboot-direct — reboot all frames sshpass -p 'lg' ssh -o StrictHostKeyChecking=no -p 2222 lg@localhost "cat > /home/lg/bin/lg-reboot-direct << 'HELPER'
...
lg-poweroff-direct — power off all frames sshpass -p 'lg' ssh -o StrictHostKeyChecking=no -p 2222 lg@localhost "cat > /home/lg/bin/lg-poweroff-direct << 'HELPER'
...
lg-refresh-set — add 2s KML refresh to slaves sshpass -p 'lg' ssh -o StrictHostKeyChecking=no -p 2222 lg@localhost "cat > /home/lg/bin/lg-refresh-set << 'HELPER'
...
lg-refresh-reset — remove KML refresh tags sshpass -p 'lg' ssh -o StrictHostKeyChecking=no -p 2222 lg@localhost "cat > /home/lg/bin/lg-refresh-reset << 'HELPER'
...
Connection refused on :2222: Cause: Tunnel down (VM mode); Fix: Laptop: ssh -N -R 2222:192.168.53.3:22 nara@<pi-ip>
Connection refused on direct IP: Cause: Wrong IP or rig off; Fix: Check IP, confirm rig powered on
Helper script not found: Cause: Not deployed; Fix: Run setup procedure
sudo -S blocked by tool guard: Cause: Pattern match in command; Fix: Write helpers on remote host, call with clean SSH
lg-relaunch does nothing: Cause: lg-ctl-master missing; Fix: Use lg-relaunch-direct instead
Slave unreachable: Cause: Physical machine off; Fix: Helpers log and skip gracefully
Reboot SSH drops (exit 255): Cause: Remote host rebooting; Fix: Normal — connection closed is expected
LAN IP drift: Cause: DHCP changes addresses; Fix: Always verify — never assume IPs from past sessions
...
Enable the relevant inbound rule for SSH
Set network profile to Private in Windows network settings
...
lg-ssh-control: Description: Relaunch, reboot, poweroff, network info, KML refresh; Status: ✅ Active
lg-kml-generator: Description: Generate and push KML from natural language; Status: ✅ Active
lg-diagnostics: Description: Health checks and recovery; Status: 📋 Planned
lg-storytelling: Description: Narrative KML tours; Status: 📋 Planned
lg-realtime-data: Description: Live feeds from OpenSky, Celestrak, weather; Status: 📋 Planned
lg-contributor: Description: Onboarding assistant for GSoC contributors; Status: 📋 Planned
...
Appended SKILL.md Sections

The two skill definitions are kept at the end of this document for easy reference in Google Docs.
...
VM / Reverse Tunnel — LG runs on a VM behind a laptop that forwards SSH via ssh -N -R 2222:192.168.53.3:22 nara@<pi-ip>. Use SSH_DEST="lg@localhost -p 2222"
Direct LAN — Real LG hardware on the same network. Use SSH_DEST="lg@<lg-master-ip>" (typically 192.168.53.3 or whatever lg1 resolves to on LAN)
...
Placemark A Placemark represents a geographic feature:
...
Styles Define reusable styles for consistent visualization:
...
Note: If kmls.txt already has other entries, append with >> not >. Use printf rather than echo to avoid leading/trailing whitespace issues.
...
Defaults: host=localhost, port=2222, password=lg (VM mode). Override with env vars: LG_HOST, LG_PORT, LG_PASSWORD. For Direct LAN mode: LG_HOST=<lg-master-ip> LG_PORT=22 bash /path/to/lg-kml-deploy.sh -f myfile.kml.
...
Working logo file on this rig: /var/www/html/kml/logo_overlay.kml Logo image: /var/www/html/kml/logo.png (8086 bytes, 200x200) URL format used by working KML: http://lg1/kml/logo.png (no :81 port) Size: 200x200 pixels Position: top-left corner (x="0" y="1")
...
Apply the fix (add refreshInterval to myplaces.kml)
Relaunch Earth ONCE (so it picks up the new myplaces.kml)
After relaunch, any write to master.kml auto-appears within N seconds — no more sed or relaunch needed
...
Run lg-refresh-set (adds refreshInterval to slave myplaces.kml)
Relaunch Earth on slaves (or the whole rig)
After relaunch, any write to slave_X.kml auto-appears within 2s — no more refresh commands needed
...
s|replace|search| — strip any existing refresh
s|search|replace| — add fresh refresh
...
Syntax Validation Use xmllint to validate KML structure:
...
Generate valid KML content using templates
Validate XML syntax (Python xml.etree or xmllint)
Deploy to /var/www/html/kmls/ via scp
Verify file is web-accessible: curl http://lg1:81/kmls/file.kml
Add URL to /var/www/html/kmls.txt for dynamic load

Updating Existing KML

Create updated version locally
Validate syntax
Deploy (overwrites existing file at /var/www/html/kmls/)
Verify — no need to modify kmls.txt (same URL, Earth re-fetches on next poll)
...
Remove content from /var/www/html/kml/master.kml
Relaunch Earth via $SSH_DEST
...
Verify the URL is in kmls.txt: cat /var/www/html/kmls.txt
Verify the file is web-accessible: curl -I http://lg1:81/kmls/file.kml (expect 200)
Check for URL typos (trailing spaces, wrong port)
Ensure the sync PHP is running: Earth polls sync_nlc.php every 1s — it doesn't need to be triggered
For a full reset: restart Earth (relaunch) to reload myplaces.kml from scratch

Connection Troubleshooting

If SSH/deployment fails:

Verify connection mode (VM/tunnel vs Direct LAN) — ask user
VM mode: check tunnel is up: ss -tlnp | grep :2222; if missing, ask laptop user for ssh -N -R 2222:192.168.53.3:22 nara@<pi-ip>
Direct LAN mode: verify ping + SSH to the master IP
Verify target directory exists: sshpass -p 'lg' ssh -o StrictHostKeyChecking=no $SSH_DEST "ls /var/www/html/kmls/"
