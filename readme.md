...
<div align="center">
  <img src="./assets/img2.png" alt="LG Logo" />
</div>
...
| Method | Code Location | Binary | Data Directory |
|--------|--------------|--------|----------------|
...
**Documentation:** For detailed installation instructions, usage guides, and developer documentation, visit: [https://hermes-agent.nousresearch.com/docs/](https://hermes-agent.nousresearch.com/docs/)
...
| KML may be inaccurate | *"AI-generated — verify key data before presenting."* |
...
<div align="center">
  <img src="/assets/img3.png" alt="Hermes Arch" />
</div>
...
<div align="center">
  <img src="/assets/img1.png" alt="hardware img" />
</div>
...
1. On your computer, download and install **Raspberry Pi Imager**.
...
> Boot order is read right-to-left: **6** = NVMe (PCIe), **1** = SD card, **f** = loop/restart. This means: try NVMe first → fall back to SD → repeat.
...
4. The system should now boot directly from the NVMe SSD. Verify with:
...
Use these commands to import profiles:
...
**`lg-refresh-reset` — remove KML refresh tags**
...
|------|-----------|-------------|
| VM / Reverse Tunnel | `SSH_DEST="lg@localhost -p 2222"` | `ss -tlnp \| grep :2222` |
...
| Reboot SSH drops (exit 255) | Remote host reboots | Expected — connection closed is normal post-reboot |
| `[sudo] password for lg:` shown despite helper | Helper pipes password via echo | `sudo -S` prints prompt to stderr |
...
Create, validate, deploy, and manage **KML** (Keyhole Markup Language) content for Liquid Galaxy rigs. This skill covers proper KML syntax, styling best practices, deployment to the Liquid Galaxy master node, and verification procedures.
...
#### **⚠️ CRITICAL: Always Include a LookAt**
...
|-------------|----------------|----------|
| **Logo** | Leftmost — Top-left corner | `slave_X.kml` (usually `slave_1`) |
| **Balloon / Index** | Rightmost — Top-right corner | `slave_X.kml` (highest index) |
...
> **Logo image:** `/var/www/html/kml/logo.png` (8086 bytes, 200×200)
> **URL format:** `http://lg1/kml/logo.png` (no `:81` port)
> **Size:** 200×200 pixels | **Position:** top-left corner (`x="0" y="1"`)
...
The `overlayXY`/`screenXY` of `x="0" y="1"` pins to top-left corner. Size 200×200 matches the working file on this rig.
