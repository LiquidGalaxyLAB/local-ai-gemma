...
- **All text content** goes to a right-screen ScreenOverlay PNG (dark bg, bullet points) — **or the news-card balloon** (dark HUD cards via `scripts/lg-kml-tours/news_card_balloon.py`, escaped HTML, auto-opened) for richer text
...
See `references/geography-educator/prebuilt-kmls.md` for the full command reference for each of these generators, including camera positions, expected file sizes, and deployment commands.
