...
# 4. Deploy slave_2.kml (ScreenOverlay) — rightmost on 3-screen rig:
cat > /tmp/slave_2.kml << 'KML'
...
## Screen Layout Rule (ROOT formula)

- Rightmost screen = floor(N/2) + 1. For 3 screens = lg2 → `slave_2.kml`
- Leftmost screen = floor(N/2) + 2. For 3 screens = lg3 → `slave_3.kml`
- The text panel ScreenOverlay goes in slave_<rightmost>.kml ONLY — never master.kml
