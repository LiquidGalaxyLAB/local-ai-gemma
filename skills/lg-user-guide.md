...
Logos and branding overlays go only on the **leftmost screen**. Formula:
```
leftRig = floor(N/2) + 2
```

| Screens | Leftmost Slave (Logo Here) |
|---------|---------------------------|
| 3 | slave_3.kml (lg3) |
| 5 | slave_4.kml (lg4) |
| 7 | slave_5.kml (lg5) |
...
descriptions go only on the **rightmost screen**. Formula:
```
rightRig = N
```

| Screens | Rightmost Slave (News/Data Here) |
|---------|--------------------------------|
| 3 | slave_3.kml (lg3) — also leftmost on 3-screen! |
| 5 | slave_5.kml (lg5) |
| 7 | slave_7.kml (lg7) |

**Important for 3-screen rigs:** On a 3-screen setup, the leftmost and
rightmost slaves are the same machine (lg3). The logo and news content are
both deployed to `slave_3.kml` — they coexist on one screen.
