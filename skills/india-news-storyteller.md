...
- `references/india-news-storyteller/news-visual-spec.md` — Full user spec: card balloon UI standard, story-type → KML-type mapping (breaking rings, country polygons, conflict layers, economy columns, disaster columns+rings, election choropleth, space arcs, multi-story pins), category color reference, implemented-vs-TODO status
