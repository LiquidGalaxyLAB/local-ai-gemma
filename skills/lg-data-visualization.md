...
└─────────────┘     └───────┬────────┘     └──────┬───────┘     └─────┬────┘
                    ...
                    └───────┬───────┘      └───────┬────────┘   │ LG screens  │
                            │                      │            │ auto-update │
                     compare_layers()     slave_3.kml gets     └─────────────┘
        ...
        └──────────┬──────────┬────────────────┘
           ...
           (source report)   (per policy)
            ...
            (✅ℹ️⚠️🔴)      + DataQualityNotes for right screen
                    ...
                    ┌───────────┴────────────┐
                    ▼                        ▼
            master.kml (main)        slave_3.kml (right screen)
...
`slave_3.kml` now includes a `📊 DATA QUALITY` section at the top, above news
...
**Collector template (see `templates/collector-base.py`):**
```python
...
Enforced by the KML writer module. See `references/kml-generation-rules.md` for the full reference.
...
- **No CDATA anywhere** — CDATA in `<description>` causes Earth VM to silently drop the entire placemark. The only safe description format is plain escaped text: `<description>Callsign: ABC123 | Altitude: 35,000 ft</description>`. This also applies to `<BalloonStyle><text>` — never use `<![CDATA[` in any KML element.
- **Icon URLs: prefer Google CDN over local PNGs.** Earth 7.3.3 VM fails to load PNGs from `http://lg1:81/kml/icons/*.png` despite HTTP 200. Use `http://maps.google.com/mapfiles/kml/shapes/air.png` or `/paddle/red-circle.png` — these render reliably.
...
```bash
# Generate 48x48 PNG icons via Pillow (see references/icon-generation.md)
...
| Logo/branding | Leftmost slave only | `slave_<N>.kml` (leftmost) | Formula: `floor(N/2)+2`, e.g. lg3 for 3-screen |
| News balloons / article index | Right screen only | `slave_<N>.kml` (rightmost) | Heavy HTML balloons don't scale well to all screens |
| Control overlays | Master only (lg1) | N/A (Earth UI) | Master has keyboard/mouse |

For N=3: lg2=left (data only, -36.5 degree yaw), lg1=center (data+control), lg3=right (data+news, +36.5 degree yaw)
...
2. `slave_3.kml` — news articles only, formatted as a right-side column

The `--data-only` flag skips news; `--news-only` skips data. Both are deployed on every run by default.

**Deploy command for each:** Python subprocess on lg1 with sudo cp, same as data deploy pattern. The slave KML goes to `/var/www/html/kml/slave_3.kml` which lg3 loads via its PHP-resolved NetworkLink.
...
# Build slave_3.kml manually via the RSSNewsCollector, or
...
When the generator crashes, master.kml is already deployed — the error only affects slave_3.kml. Generate a minimal news list separately:
```python
...
See `references/worldmonitor-sources.md` for a catalog of ~20 additional
...
1. Check `references/worldmonitor-sources.md` for a known free API
2. If not listed, search for `open source <topic> api free` or check
   `references/dual-source-apis.md` for secondary-source patterns
...
- KML generation conventions in `references/kml-generation-rules.md`
- Screen placement decisions in `references/screen-placement.md`
...
- `references/kml-generation-rules.md` — Complete KML ruleset for data visualization
- `references/screen-placement.md` — What goes where on which screen (LG Wiki + Nara conventions)
- `references/dual-source-comparison.md` — Dual-source architecture, proximity matching algorithm, status levels, adding new v2 collectors
- `references/symbol-icons.md` — Google Maps KML icon palette mappings
- `references/usgs-earthquakes.md` — USGS earthquake feed: URL, response format, KML generation pattern
- `references/eonet-natural-events.md` — NASA EONET natural events API
- `references/noaa-weather.md` — NOAA NWS weather alerts API
- `references/gdacs-disasters.md` — GDACS global disaster events API
- `references/military-bases-config.md` — Static military bases configuration
- `references/vm-startup-pitfall.md` — VM Earth startup fix (launch-earth.sh hangs on unreachable slaves)
- `references/opensky-air-traffic.md` — OpenSky API, ICAO hex prefixes, rate limits
- `references/rss-news-feeds.md` — RSS news aggregation, topic classification, fan-out geo-positioning
- `references/icon-generation.md` — Custom PNG icon generation via Pillow (16 icons, heading rotation support)
- `references/dual-source-apis.md` — Secondary source APIs: EMSC endpoint quirks (format=json NOT geojson, ISO time not Unix), wttr.in WMO code mapping, Wikipedia Current Events parser approach with place-based geo-location
- `references/fusion-engine.md` — Fusion engine API: FusionPolicy, DataQualityNote, FusionResult, confidence encoding, status logic, adding new policies for new layers
- `references/worldmonitor-sources.md` — Catalog of ~30 data layers from the World Monitor (74k star) project including ACLED conflict events, NASA FIRMS wildfires, AISStream ship tracking, NGA maritime warnings, Safecast radiation, HDX humanitarian data — with verified-working URLs and auth requirements for each
- `references/dual-source-apis.md` — Secondary source APIs (EMSC, wttr.in, Wikipedia Current Events), source comparison logic, disclaimer overlay pattern

## Templates
- `templates/collector-base.py` — Base Python collector scaffold with KML generation and deploy
- `templates/placeholder.kml` — Minimal valid KML with LookAt (start here for any test)
