...
def make_text_panel(zone):
    """Generate right-screen text panel for current zone."""
    ...
    for bi in range(5):
        ...
        draw.rectangle([(bx, by), (bx + 22, by + 18)], fill=col)
    
    y += 35
    draw.text((16, y), "Location: " + str(round(zone["lat"], 1)) + "N, " + str(round(zone["lon"], 1)) + "E", font=fs, fill=(150, 150, 180))
    y += 22
    draw.text((16, y), "Affected: " + str(zone.get("pop", "N/A")), font=fs, fill=(150, 150, 180))
    
    # Status indicator
    y += 35
    status_colors = {"war": (255,0,0), "siege": (255,60,0), "crisis": (255,120,0), 
                     "conflict": (255,160,0), "dispute": (200,200,50)}
    sc = status_colors.get("war", (255,255,255))
    draw.ellipse([(16, y), (30, y+14)], fill=sc)
    draw.text((38, y), "ACTIVE CONFLICT", font=fs, fill=(255, 255, 255))
    
    y += 30
    draw.line([(16, y), (484, y)], fill=(60, 60, 80), width=1)
    y += 10
    draw.text((16, y), "Source: ACLED + BBC Monitoring", font=ImageFont.load_default(), fill=(100, 100, 120))
...
def deploy_panel(path):
    """SCP and deploy text panel PNG to right screen."""
    subprocess.run(['sshpass', '-p', LG_PASS, 'scp', '-o', 'StrictHostKeyChecking=no',
        path, 'lg@' + LG_IP + ':/home/lg/rp.png'], capture_output=True, timeout=30)
    subprocess.run(['sshpass', '-p', LG_PASS, 'ssh', '-o', 'StrictHostKeyChecking=no',
        'lg@' + LG_IP, 'echo ' + LG_PASS + ' | sudo -S cp /home/lg/rp.png /var/www/html/kml/right_panel.png 2>/dev/null'],
...
def build_kml():
    ...
    for i, z in enumerate(ZONES):
        ...
        features += (
            '<Placemark><name>' + z["label"] + '</name>'
            '<styleUrl>#' + s + '_txt</styleUrl>'
            '<Point><coordinates>' + str(lon+1.8) + ',' + str(lat+0.8) + ',0</coordinates></Point>'
            '</Placemark>\n'
        )
        
        if "ukraine" in n:
            styles += '<Style id="' + s + '_l"><LineStyle><color>ffff0000</color><width>6</width></LineStyle>\n'
            pts = [(0,0),(2,1),(4,0),(6,1),(8,0)]
            line = ' '.join(str(lon+o)+','+str(lat+o2)+',0' for o,o2 in pts)
...
def main():
    ...
    print("  Waiting for KML...")
    time.sleep(8)
    print("  KML deployed.\n")
    
    # Deploy initial text panel
    deploy_panel(make_text_panel(ZONES[0]))
    
    # Camera tour with synced text + voice
    ...
    for z in ZONES:
        print("    " + z["name"])
        # 1. Fly to zone
        fly_to(z["lon"], z["lat"], 600000, 55)
        time.sleep(2)
        # 2. Update right-screen text panel
        deploy_panel(make_text_panel(z))
        time.sleep(1)
        # 3. Voiceover + dwell
        print("      VOICE: " + z["desc"][:100])
        time.sleep(10)  # Dwell for voiceover
