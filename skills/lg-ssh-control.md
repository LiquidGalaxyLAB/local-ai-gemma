...
Each screen displays the same `master.kml` Earth visualization (via ViewSync sync). Additional overlay content is layered per screen:

| Screen | Physical Position | Overlay | Slave KML |
|--------|------------------|---------|-----------|
| lg1 (master) | Center | None (clean Earth view) | N/A |
| lg2 (slave) | Left | **Logo** (top-left, ScreenOverlay PNG) | `slave_2.kml` |
| lg3 (slave) | Right | **Text info panel** (right edge, ScreenOverlay PNG) | `slave_3.kml` |

**Frame-count-agnostic formula** (for N screens):
- Leftmost slave for logo: `floor(N/2) + 2`
- Rightmost slave for text panel: `N`

On a 3-screen rig: leftmost = lg3, rightmost = lg3 (same machine). Both logo and text panel coexist in `slave_3.kml`. The current rig separates them across `slave_2.kml` (logo) and `slave_3.kml` (text panel) due to the physical screen layout.

**Key rule:** All text content (titles, explanations, bullet points, data quality) goes to the right-screen text panel PNG — never as placemark labels on the Earth globe. The Earth KML (`master.kml`) should contain only visual elements: points, lines, polygons, icons.
...
| **Slave Solo KML loads once at startup — never re-polls for updates** | **The Solo KML NetworkLink on lg2/lg3 (`slave_x.kml`) has no `<refreshInterval>`. Slave loads the KML once and never picks up changes to slave_3.kml.** | **Add refreshInterval via sed on each slave: `sed -i 's|<href>http://lg1:81/kml/slave_3.kml</href>|<href>http://lg1:81/kml/slave_3.kml</href>\\n\\t\\t\\t\\t<refreshMode>onInterval</refreshMode>\\n\\t\\t\\t\\t<refreshInterval>3</refreshInterval>|' ~/.googleearth/myplaces.kml` then relaunch Earth on that slave.** |
...
| **Earth 7.3.3 Signal 11 crash in libxcb - with or without --no_system_check** | On VirtualBox, Earth 7.3.3 segfaults (Signal 11) in the xcb/Qt5 graphics stack during 3D rendering. --no_system_check suppresses the unknown graphics card dialog, but xcb crash can still occur with GPU acceleration. Breaks NetworkLink subsystem - Earth appears running but makes zero HTTP requests. | **Fix: LIBGL_ALWAYS_SOFTWARE=1 + LIBGL_ALWAYS_INDIRECT=1 in addition to --no_system_check --no_signin. Launcher at /home/lg1/lg-launcher2.sh sets all three.** |
